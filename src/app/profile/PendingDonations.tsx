...
export default function PendingDonations({ interests }: { interests: Interest[] }) {
  ...
  return (
    <>
      <div className="section-label" style={{ padding: '24px 22px 8px' }}>Pending donations</div>
      <div style={{ padding: '0 22px', display: 'flex', flexDirection: 'column', gap: 9 }}>
        {items.map(interest => (
          <div key={interest.id} className="card" style={{ padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 11 }}>
            <IconTile size={34} radius={10} background="var(--warn-bg)"><span style={{ fontSize: 18 }}>📦</span></IconTile>
